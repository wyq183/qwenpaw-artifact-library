...
(function () {
  ...
  function request(path, opts) { return Q.host.fetch("/artifact-library" + path, opts).then(function (r) { return r.json().then(function (body) { if (!r.ok) throw new Error(body.detail || "请求失败"); return body; }); }); }
  ...
  function ArtifactLibrary() {
    ...
    var loadGenerated = React.useCallback(function(){ setBusy(true); var p = new URLSearchParams(); if(query)p.set("query",query); if(genModel)p.set("model_name",genModel); if(genLora)p.set("lora_name",genLora); if(genRating)p.set("min_rating",genRating); if(genSort)p.set("sort",genSort); request("/generated-images?" + p.toString()).then(function(d){ setGenItems(d.items || []); setFacets(d.facets || {}); }).catch(function(e){ message.error(e.message); }).finally(function(){setBusy(false);}); }, [query, genModel, genLora, genRating, genSort]);
    ...
    var sendToImageGen = function(item) { post("/generated-images/" + item.id + "/send-to-image-gen").then(function(d){ if (d.success) message.success("已发送到生图助手草稿"); else message.warning(d.error || "发送失败"); }).catch(function(e){ message.error(e.message); }); };
    ...
    var importGenerated = function(){ var proj = prompt("导入到哪个项目？", "生图图库"); if (!proj) return; post("/generated-images/import", {project:proj, limit:0}).then(function(d){message.success("导入 " + d.imported + " 张，跳过 " + d.skipped + " 张"); loadGenerated();}).catch(function(e){message.error(e.message);}); };
    ...
    var renderGenerated = function(){ return h("div", null,
      ...
      genItems.length ? h("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14}}, genItems.map(function(x){ var m=GenMeta(x); return h(Card,{key:x.id,hoverable:true,onDoubleClick:function(){openDetail(x);},cover:h("img",{src:"/artifact-library/artifacts/"+x.id+"/thumbnail",style:{height:190,objectFit:"cover"},onError:function(e){e.currentTarget.style.display='none';}})},
        ...
        h("div",{style:{marginTop:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}, h(Rate,{disabled:true,value:Number(m.rating||0),style:{fontSize:14}}), h(Button,{size:"small",onClick:function(e){e.stopPropagation();copyText(m.prompt,"Prompt");}},"复制Prompt")),
    ...
    return h("div",{style:{height:"100%",minHeight:"100%",background:"var(--ant-color-bg-layout,#f5f5f5)",padding:"28px 32px",boxSizing:"border-box"}}, h("div",{style:{maxWidth:1500,margin:"0 auto"}},
      ...
      h("section",{style:{background:"var(--ant-color-bg-container,#fff)",border:"1px solid var(--ant-color-border-secondary,#f0f0f0)",borderRadius:12,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,.025)"}},
        h("div",{style:{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:16}},
          ...
          h(Radio.Group,{value:view,onChange:function(e){setView(e.target.value);setSelectedRowKeys([]);},optionType:"button",buttonStyle:"solid",options:[{label:"列表",value:"list"},{label:"卡片",value:"cards"},{label:"项目",value:"projects"},{label:"生图库",value:"generated"}]}),
