...
- 适配生图助手 v1.2.1 起的数据库新位置（`%APPDATA%\QwenPaw\qwenpaw-image-gen\`），探测顺序中**新位置优先**。
- 旧位置继续保留在候选列表，未升级的生图助手仍可正常联动。

### v0.5.5 生图失效记录管理

- 生图库独立统计失效记录：标题与生图库视图直接显示「失效 N 张」，不再只统计普通产物。
- 新增「同步修复路径」入口：生图助手迁移存储目录后，可一键按 source_id 原地修复记录路径，不必逐条重建。
- 新增「清理失效记录」：后端 `/generated-images/cleanup-missing` 按全库范围清理原文件已不存在的生图记录，只标记回收站、不动磁盘文件，且不受当前筛选条件影响。
- 新增「隐藏失效」开关：可选只显示原文件仍存在的生图资产，便于专注可用素材。
- 生图库卡片新增「文件丢失」角标，缩略图半透明显示，失效记录一眼可辨。
- 生图库排序补全为最新生成 / 最早生成 / 星级优先 / 按模型 / 文件最大，与后端排序能力对齐。

### v0.5.4 生图详情增强

- 生图库卡片增加「复制负向」按钮，正向/负向提示词可一键复制。
- 详情页直接展示正向、负向提示词原文，便于复刻与二次创作。

### v0.5.2 缩略图实时缩放与隐私修复

- 缩略图改为实时生成：后端用 PIL 等比缩放到 480px 宽直出 JPEG，与详情图 100% 一致，不依赖独立缓存。
- 缩略图缓存 key 加入 artifact id，避免不同记录指向同名文件时串图；缓存超过 72 小时自动重建。
- 图片接口响应头改为 no-cache，杜绝旧图缓存残留。
- 社区包检查改用正则白名单与通用隐私模式，移除个人化关键词；测试模型名改用通用示例名。

### v0.5.1 排序与界面修复

- 生图库按生图助手真实生成时间排序，不再按导入时间排序。
- 删除无实际作用的“送回生图助手”功能，保留 Prompt、负向提示词和参数摘要复制。
- 视图切换优化为普通视图与独立生图库入口。

### v0.5.0 生图助手分类同步

- 从生图助手导入时同步每张图片的自定义分类（未设置时为“未分类”）。
- 生图库新增 **生图分类** 筛选器，显示分类名称及图片数量。
- 再次同步会读取生图助手的最新分类；如果用户在生图助手中移动图片分类，产物库会在下一次同步时更新对应记录。
- 分类实行 **生图助手 → 产物库** 单向同步：生图助手是唯一分类来源，产物库不反向修改生图助手分类，避免双端状态冲突。
- 生图库卡片显示当前同步的分类，仍可与模型、LoRA、星级和关键词组合筛选。

### v0.4.7 缓存与版本一致性

- 前端入口随版本变化，例如 `ui/index.v0.5.0.js`，避免 WebView 命中旧静态资源。
- 后端提供不缓存的运行版本接口；前端启动后会核对自身版本与实际后端版本。
- 发现版本不一致时自动刷新一次；仍不一致会明确提示用户完全退出并重新打开 QwenPaw，避免新旧代码静默混用。
- 插件启动时只清理非当前版本的版本化前端文件，并带完整性安全阀：当前入口不存在时绝不删除任何前端文件。

### 安装与更新（重要）

本插件采用**完整冷更新**，不支持运行中覆盖或热更新：

1. 完全退出 QwenPaw Desktop。
2. 在插件管理中卸载旧版产物库。
3. 安装新版 ZIP 包。
4. 重新打开 QwenPaw Desktop。
5. 打开产物库，确认标题显示的版本号与安装包一致。
